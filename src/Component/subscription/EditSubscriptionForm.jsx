...
function EditSubscriptionForm() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const response = await axios.put(`${baseUrl}/subscriptions/update/${id}`,
                updatedData,
            );
    ...
    const fetchItemDetails = async () => {
        try {
            const response = await axios.get(`${baseUrl}/subscriptions/details/${id}`);
            ...
            setitemname(ItemData.itemName || '');
            setSelectedPeriods(ItemData.period || []);
            setSelectedtimePeriods(ItemData.timings || []);

            // Set size-related values
            if (ItemData.size) {
                setsizeId(ItemData.size.sizeId || '');
                // Use setTimeout to ensure refs are available
                setTimeout(() => {
                    if (volume.current) volume.current.value = ItemData.size.volume || '';
                    if (sizePrice.current) sizePrice.current.value = ItemData.size.sizePrice || '';
                }, 0);
            }
        ...
        <div className="dashboard-container">
            <div className="col-md-12 main-content">
                <div className="form-container">
                    ...
                    <h1 className="form-title">Edit Subscription</h1>
